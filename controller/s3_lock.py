...
class S3Lock:
    ...
    def __init__(
        ...
        if process_num not in [self.PROCESS_0, self.PROCESS_1]:
            raise S3LockBadProcessName(
                f"Process name must be str {self.PROCESS_0} or {self.PROCESS_1}"
            )
    ...
    def init(self, overwrite: bool = False):
        ...
        for lock_file in lock_files:
            if overwrite or not self._s3_object_exists(self.FLAG_0_FILE):
                ...
                if self.verbose:
                    print(
                        f"Not writing file '{lock_file}' - overwrite false and file exists."
                    )
    ...
    def wait_for_lock(self):
        ...
        if self.verbose:
            ...
            print(
                f"Setting {self._my_flag_file} and setting turn to {self._their_turn_value}."
            )
        ...
        while self._they_want_to_enter and self._its_their_turn:
            ...
            if self.verbose:
                ...
                print(
                    f"Poor little old me, process {self.process_num}, waiting for my turn for {waited_for} seconds.."
                )
    ...
    def _my_flag_file(self):
        return (
            self.FLAG_0_FILE if self.process_num == self.PROCESS_0 else self.FLAG_1_FILE
        )

    @property
    def _their_flag_file(self):
        return (
            self.FLAG_0_FILE if self.process_num == self.PROCESS_1 else self.FLAG_1_FILE
        )
    ...
    def _their_turn_value(self):
        return (
            self.P0_TURN_VALUE
            if self.process_num == self.PROCESS_1
            else self.P1_TURN_VALUE
        )

    @property
    def _all_files_exist(self):
        return (
            self._s3_object_exists(self.FLAG_0_FILE)
            and self._s3_object_exists(self.FLAG_1_FILE)
            and self._s3_object_exists(self.TURN_FILE)
        )
